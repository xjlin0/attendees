...
class Price(TimeStampedModel, SoftDeletableModel, TimeFramedModel, Utility):
    ...
    meet = models.ForeignKey(Meet, null=True, on_delete=models.SET_NULL)
    ...
    def __str__(self):
        return '%s %s %s %s %s' % (self.meet, self.display_name, self.start, self.price_type, self.price_value)
